...
#include "imgproc/int_literals.h"
...
namespace nvimgcodec {
    ...
    NVIMGCODEC_HOST_DEV constexpr T* begin(T (&array)[N]) noexcept { return array; }
    template <typename T, size_t N>
    NVIMGCODEC_HOST_DEV constexpr T* end(T (&array)[N]) noexcept { return array + N; }

    NVIMGCODEC_NO_EXEC_CHECK
    template <typename T, typename = std::enable_if_t<!std::is_const<T>::value>>
    NVIMGCODEC_HOST_DEV constexpr auto begin(T& collection) -> decltype(collection.begin()) {
        return collection.begin();
    }

    NVIMGCODEC_NO_EXEC_CHECK
    template <typename T, typename = std::enable_if_t<!std::is_const<T>::value>>
    NVIMGCODEC_HOST_DEV constexpr auto end(T& collection) -> decltype(collection.end()) {
        return collection.end();
    }

    NVIMGCODEC_NO_EXEC_CHECK
    template <typename T>
    NVIMGCODEC_HOST_DEV constexpr auto begin(const T& collection) -> decltype(collection.begin()) {
        return collection.begin();
    }

    NVIMGCODEC_NO_EXEC_CHECK
    template <typename T>
    NVIMGCODEC_HOST_DEV constexpr auto end(const T& collection) -> decltype(collection.end()) {
        return collection.end();
    }

    NVIMGCODEC_NO_EXEC_CHECK
    template <typename Target, typename Source>
    NVIMGCODEC_HOST_DEV void append(Target& target, const Source& source) {
        target.insert(nvimgcodec::end(target), nvimgcodec::begin(source), nvimgcodec::end(source));
    }

    NVIMGCODEC_NO_EXEC_CHECK
    template <typename Collection>
    NVIMGCODEC_HOST_DEV auto size(const Collection& c) -> decltype(c.size()) {
        return c.size();
    }
    ...
    NVIMGCODEC_HOST_DEV constexpr Value align_up(Value v, Alignment a) {
        return v + ((a - 1) & -v);
    }

    template <typename Value, typename Alignment>
    NVIMGCODEC_HOST_DEV constexpr Value alignment_offset(Value v, Alignment a) {
        return v & (a - 1);
    }

    template <typename Value, typename Alignment>
    NVIMGCODEC_HOST_DEV constexpr Value align_down(Value v, Alignment a) {
        return v & -a;
    }
    ...
    template <typename T>
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr std::enable_if_t<std::is_integral<T>::value, T> next_pow2(T n) {
        T pow2 = 1;
        while (n > pow2) {
            pow2 += pow2;
        }
    ...
    template <typename T>
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr std::enable_if_t<std::is_integral<T>::value, T> prev_pow2(T n) {
        T pow2 = 1;
    ...
    template <typename T>
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr std::enable_if_t<std::is_integral<T>::value, bool> is_pow2(T n) {
        return (n & (n - 1)) == 0;
    }
     ...
     */
    template <typename T>
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr std::enable_if_t<std::is_integral<T>::value, int> ilog2(T x) {
        int n = 0;
        while (x >>= 1)
            n++;
        return n;
    ...
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE
        OutType
        to_bit_mask(const BitIndices& bit_indices) {
    ...
    using if_iterable = if_istype<decltype(*nvimgcodec::end(std::declval<Collection>())),
                                  if_istype<decltype(*nvimgcodec::begin(std::declval<Collection>())), T>>;
    ...
    using if_array_like = if_indexable<Collection,
                                       if_istype<decltype(nvimgcodec::size(std::declval<Collection>())), T>>;
    ...
    using is_integer_collection = is_integer_iterator<
        decltype(nvimgcodec::begin(std::declval<C>()))>;
...
#define IMPL_HAS_UNIQUE_STATIC_FUNCTION(function_name)                                        \
    ...
    template <typename T>                                                                     \
    struct has_unique_static_function_##function_name : decltype(HasUniqueStaticFunction_##function_name<T>(nullptr)) {};
...
#define IMPL_HAS_UNIQUE_MEMBER_FUNCTION(function_name)            \
    ...
    template <typename T>                                         \
    struct has_unique_member_function_##function_name : decltype(HasUniqueMemberFunction_##function_name<T>(nullptr)) {};
    ...
    NVIMGCODEC_HOST_DEV inline volume_t<decltype(*std::declval<Iter>())>
    ...
    NVIMGCODEC_HOST_DEV inline auto volume(const Shape& shape) -> decltype(volume(nvimgcodec::begin(shape), nvimgcodec::end(shape))) {
        return volume(nvimgcodec::begin(shape), nvimgcodec::end(shape));
    }
    ...
    template <typename Extent>
    NVIMGCODEC_HOST_DEV constexpr volume_t<Extent> volume(Extent single_dim) {
        return single_dim;
    }
