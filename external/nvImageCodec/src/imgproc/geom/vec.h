...
#endif
#include "imgproc/force_inline.h"
...
#define DEFINE_VEC_ALIASES(prefix)       \
    ...
    struct vec_base {
        ...
        NVIMGCODEC_HOST_DEV constexpr vec_base(Components... components) : v{T(components)...} {} // NOLINT
        T v[N];
    };
    ...
    struct vec : vec_base<N, T> {
        ...
        template <typename... Components,
                  typename = std::enable_if_t<sizeof...(Components) == N>>
        NVIMGCODEC_HOST_DEV constexpr vec(Components... components) : vec_base<N, T>(components...) {} // NOLINT

        using vec_base<N, T>::v;

        template <typename U>
        NVIMGCODEC_HOST_DEV explicit constexpr vec(const mat<N, 1, U>& m) : vec(m.col(0).template cast<T>()) {}

        NVIMGCODEC_HOST_DEV
        constexpr vec(const mat<N, 1, T>& m) : vec(m.col(0).template cast<T>()) {} // NOLINT

        template <typename U>
        NVIMGCODEC_HOST_DEV explicit constexpr vec(const vec<N, U>& v) : vec(v.template cast<T>()) {}

        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr T& operator[](int i) { return v[i]; }
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr const T& operator[](int i) const { return v[i]; }

        template <typename U>
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr vec<N, U> cast() const {
            vec<N, U> ret = {};
            for (int i = 0; i < N; i++) {
        ...
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr int size() const { return N; }

        NVIMGCODEC_HOST_DEV constexpr T* begin() { return &v[0]; }
        NVIMGCODEC_HOST_DEV constexpr const T* cbegin() const { return &v[0]; }
        NVIMGCODEC_HOST_DEV constexpr const T* begin() const { return &v[0]; }
        NVIMGCODEC_HOST_DEV constexpr T* end() { return &v[N]; }
        NVIMGCODEC_HOST_DEV constexpr const T* cend() const { return &v[N]; }
        NVIMGCODEC_HOST_DEV constexpr const T* end() const { return &v[N]; }

        /// @brief Calculates the sum of squares of components.
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr auto length_square() const {
        ...
        NVIMGCODEC_HOST_DEV inline auto length() const {
#ifdef __CUDA_ARCH__
        ...
        NVIMGCODEC_HOST_DEV inline vec normalized() const {
            auto lsq = length_square();
            return *this * rsqrt(lsq);
        }

        /// @brief Returns a copy. Doesn't promote type to int.
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr vec operator+() const { return *this; }

        /// @brief Negates all components. Doesn't promote type to int.
        NVIMGCODEC_HOST_DEV
        NVIMGCODEC_FORCEINLINE constexpr vec operator-() const {
            vec<N, T> ret{};
            for (int i = 0; i < N; i++) {
                ret.v[i] = -v[i];
            }
...
#define DEFINE_ASSIGN_VEC_OP(op)                                                        \
    template <typename U>                                                               \
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE vec& operator op(const vec<N, U>& rhs) { \
        for (int i = 0; i < N; i++)                                                     \
            v[i] op rhs[i];                                                             \
        return *this;                                                                   \
    }                                                                                   \
...
#undef DEFINE_ASSIGN_VEC_OP
    ...
    NVIMGCODEC_HOST_DEV
        NVIMGCODEC_FORCEINLINE constexpr auto
        dot(const vec<N, T>& a, const vec<N, U>& b) {
    ...
    NVIMGCODEC_HOST_DEV
        NVIMGCODEC_FORCEINLINE constexpr auto
        cross(const vec<3, T>& a, const vec<3, U>& b) {
    ...
    NVIMGCODEC_HOST_DEV
        NVIMGCODEC_FORCEINLINE constexpr auto
        cross(const vec<2, T>& a, const vec<2, U>& b) {
        return a.x * b.y - b.x * a.y;
    }

#define DEFINE_ELEMENTIWSE_VEC_BIN_OP(op)                                                                              \
    template <int N, typename T, typename U>                                                                           \
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr auto operator op(const vec<N, T>& a, const vec<N, U>& b) {    \
...
#define DEFINE_SHIFT_VEC_BIN_OP(op)                                                                                      \
    template <int N, typename T, typename U>                                                                             \
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr vec<N, T> operator op(const vec<N, T>& a, const vec<N, U>& b) { \
        vec<N, T> ret{};                                                                                                 \
        for (int i = 0; i < N; i++)                                                                                      \
            ret[i] = a[i] op b[i];                                                                                       \
        return ret;                                                                                                      \
    ...
    struct is_true {
        template <typename T>
        NVIMGCODEC_HOST_DEV constexpr bool operator()(const T& x) {
            return static_cast<bool>(x);
        }
    };

    template <int N, typename T, typename Pred = is_true>
    NVIMGCODEC_HOST_DEV constexpr bool all_coords(const vec<N, T>& a, Pred P = {}) {
        for (int i = 0; i < N; i++)
            if (!P(a[i]))
                return false;
        return true;
    ...
    template <int N, typename T, typename Pred = is_true>
    NVIMGCODEC_HOST_DEV constexpr bool any_coord(const vec<N, T>& a, Pred P = {}) {
        for (int i = 0; i < N; i++)
            if (P(a[i]))
                return true;
        return false;
    ...
    template <int N, typename T, typename U>
    NVIMGCODEC_HOST_DEV constexpr bool operator==(const vec<N, T>& a, const vec<N, U>& b) {
        for (int i = 0; i < N; i++)
            if (a[i] != b[i])
                return false;
        return true;
...
#define IMPL_VEC_ELEMENTWISE(...)                                               \
    ...
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr vec<N, To> cast(const vec<N, From>& v) {
        return v.template cast<To>();
    }

    template <int N, typename T>
    NVIMGCODEC_HOST_DEV constexpr vec<N, T>
    ...
    template <int N>
    constexpr NVIMGCODEC_DEVICE vec<N> floor(const vec<N>& a) {
        IMPL_VEC_ELEMENTWISE(floorf(a[i]));
    }

    template <int N>
    constexpr NVIMGCODEC_DEVICE vec<N> ceil(const vec<N>& a) {
        IMPL_VEC_ELEMENTWISE(ceilf(a[i]));
    }

    template <int N, typename T>
    constexpr NVIMGCODEC_DEVICE vec<N, T> min(const vec<N, T>& a, const vec<N, T>& b) {
        IMPL_VEC_ELEMENTWISE(::min(a[i], b[i]));
    }

    template <int N, typename T>
    constexpr NVIMGCODEC_DEVICE vec<N, T> max(const vec<N, T>& a, const vec<N, T>& b) {
    ...
    template <int N, typename T>
    constexpr NVIMGCODEC_HOST vec<N, T> floor(const vec<N, T>& a) {
        IMPL_VEC_ELEMENTWISE(std::floor(a[i]));
    }

    template <int N, typename T>
    constexpr NVIMGCODEC_HOST vec<N, T> ceil(const vec<N, T>& a) {
        IMPL_VEC_ELEMENTWISE(std::ceil(a[i]));
    }

    template <int N, typename T>
    constexpr NVIMGCODEC_HOST vec<N, T> min(const vec<N, T>& a, const vec<N, T>& b) {
        IMPL_VEC_ELEMENTWISE(std::min(a[i], b[i]));
    }

    template <int N, typename T>
    constexpr NVIMGCODEC_HOST vec<N, T> max(const vec<N, T>& a, const vec<N, T>& b) {
    ...
    template <int N>
    NVIMGCODEC_HOST_DEV ivec<N> round_int(const vec<N>& a) {
        IMPL_VEC_ELEMENTWISE(round_int(a[i]));
    }

    template <int N>
    NVIMGCODEC_HOST_DEV ivec<N> floor_int(const vec<N>& a) {
        IMPL_VEC_ELEMENTWISE(floor_int(a[i]));
    }

    template <int N>
    NVIMGCODEC_HOST_DEV ivec<N> ceil_int(const vec<N>& a) {
        IMPL_VEC_ELEMENTWISE(ceil_int(a[i]));
    }

    template <int N, typename T, typename U>
    NVIMGCODEC_HOST_DEV constexpr std::enable_if_t<std::is_integral<T>::value && std::is_integral<U>::value,
    ...
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr auto cat(const vec<size0, T>& v0, const vec<size1, T>& v1) {
    ...
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr auto cat(const vec<size0, T>& v0, T v1) {
        vec<size0 + 1, T> ret = {};
        for (int i = 0; i < size0; i++) {
            ret[i] = v0[i];
        }
    ...
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr auto cat(T v0, const vec<size1, T>& v1) {
        vec<size1 + 1, T> ret = {};
        ret[0] = v0;
        for (int i = 0; i < size1; i++) {
            ret[i + 1] = v1[i];
    ...
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr auto cat(const vec<size0, T>& v0, const vec<sizes, T>&... tail) {
        return cat(v0, cat(tail...));
    }

    template <int sub_n, int n, typename T>
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr auto sub(const vec<n, T>& orig, int start = 0) {
    ...
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr vec<sizeof...(indices), T> shuffle(const vec<N, T>& v) {
...
__device__ NVIMGCODEC_FORCEINLINE nvimgcodec::vec<N, T> __ldg(const nvimgcodec::vec<N, T>* ptr) {
