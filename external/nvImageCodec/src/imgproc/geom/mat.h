...
#include "imgproc/cuda_utils.h"
#include "imgproc/force_inline.h"
#include "imgproc/geom/vec.h"
#include "imgproc/host_dev.h"
#include "imgproc/tuple_helpers.h"
...
#define MAT_STORAGE_LOOP(m_index) \
    for (int m_index = 0; m_index < cols; m_index++)
        ...
        NVIMGCODEC_HOST_DEV constexpr mat(const vec<rows, U>& v) : m{} { // NOLINT
        ...
        template <typename U>
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr mat(const mat<rows, cols, U>& rhs) : mat(rhs.template cast<Element>()) { // NOLINT
        }

        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr auto row(int r) const {
#if MAT_LAYOUT_ROW_MAJOR
            return m[r];
#else
        ...
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr auto col(int c) const {
#if MAT_LAYOUT_ROW_MAJOR
        ...
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE void set_col(int c, const col_t& col) {
#if MAT_LAYOUT_ROW_MAJOR
            for (int i = 0; i < rows; i++)
                m[i][c] = col[i];
#else
        ...
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE void set_row(int r, const row_t& row) {
#if MAT_LAYOUT_ROW_MAJOR
            m[r] = row;
#else
            for (int j = 0; j < cols; j++)
        ...
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr Element& at(int r, int c) {
#if MAT_LAYOUT_ROW_MAJOR
            return m[r][c];
#else
            return m[c][r];
        ...
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr const Element& at(int r, int c) const {
#if MAT_LAYOUT_ROW_MAJOR
            return m[r][c];
#else
            return m[c][r];
        ...
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr Element& operator()(int r, int c) {
            return at(r, c);
        }
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr const Element& operator()(int r, int c) const {
            return at(r, c);
        }
        ...
        template <typename U>
        NVIMGCODEC_HOST_DEV constexpr mat<rows, cols, U> cast() const {
        ...
        NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr mat operator+() const {
            return *this;
        }
...
#undef DEFINE_ASSIGN_MAT_SCALAR_OP
        ...
        NVIMGCODEC_HOST_DEV inline mat& operator*=(const mat<cols, rhs_cols, U>& m) {
        ...
        NVIMGCODEC_HOST_DEV constexpr auto operator*(const mat<cols, rhs_cols, U>& rhs) const {
        ...
        template <typename U>
        NVIMGCODEC_HOST_DEV constexpr auto operator*(const vec<cols, U>& v) const {
            using R = promote_vec_t<Element, U>;
#if MAT_LAYOUT_ROW_MAJOR
            vec<rows, R> result{};
            for (int i = 0; i < rows; i++) {
         ...
         */
        NVIMGCODEC_HOST_DEV static constexpr mat diag(Element scalar) {
            return mat(scalar);
        }

        /**
         * @return nvimgcodec::mat with ones on the main diagonal and zeros elsewhere
         */
        NVIMGCODEC_HOST_DEV static constexpr mat eye() {
            return diag(1);
        }

        NVIMGCODEC_HOST_DEV static constexpr mat identity() {
            return diag(1);
        }
    };

    template <int rows, int cols, typename T, typename U>
    NVIMGCODEC_HOST_DEV constexpr bool operator==(const mat<rows, cols, T>& a, const mat<rows, cols, U>& b) {
        MAT_ELEMENT_LOOP(i, j)
        if (a(i, j) != b(i, j))
            return false;
        return true;
    ...
    NVIMGCODEC_HOST_DEV NVIMGCODEC_FORCEINLINE constexpr auto sub(const mat<in_rows, in_cols, Element>& m, int r = 0, int c = 0) {
...
#define DEFINE_ELEMENTWISE_MAT_MAT_BINOP(op)                                       \
    ...
    NVIMGCODEC_HOST_DEV constexpr auto operator op(const mat<rows, cols, T1>& a,   \
...
#define DEFINE_MAT_ALIASES(rows, cols)            \
    ...
    NVIMGCODEC_HOST_DEV constexpr mat<rows, c1 + c2, T> cat_cols(const mat<rows, c1, T>& a, const mat<rows, c2, T>& b) {
    ...
    NVIMGCODEC_HOST_DEV constexpr mat<rows, cols + 1> cat_cols(const mat<rows, cols, T>& a, const vec<rows, T>& v) {
    ...
    template <typename T, int rows, int cols>
    NVIMGCODEC_HOST_DEV constexpr mat<rows, cols + 1> cat_cols(const vec<rows, T>& v, const mat<rows, cols, T>& a) {
        mat<rows, cols + 1, T> ret{};
#if MAT_LAYOUT_ROW_MAJOR
        for (int i = 0; i < rows; i++)
    ...
    NVIMGCODEC_HOST_DEV constexpr mat<rows, 2> cat_cols(const vec<rows, T>& a, const vec<rows, T>& b) {
        mat<rows, 2, T> ret{};
#if MAT_LAYOUT_ROW_MAJOR
        for (int i = 0; i < rows; i++)
    ...
    NVIMGCODEC_HOST_DEV constexpr mat<r1 + r2, cols, T> cat_rows(const mat<r1, cols, T>& a, const mat<r2, cols, T>& b) {
    ...
    std::ostream& operator<<(std::ostream& os, const nvimgcodec::mat<rows, cols, T>& m) {
    ...
    NVIMGCODEC_HOST_DEV inline void solve_gauss(mat<N, N, T>& A, mat<N, M, T>& B) {
        for (int v = 0; v < N; v++) {
            T max = std::abs(A(v, v));
            int maxr = v;
            for (int i = v + 1; i < N; i++) {
    ...
    NVIMGCODEC_HOST_DEV inline mat<n, n, U> inverse(mat<n, n, T> A) {
        mat<n, n, U> O = 1;
    ...
    NVIMGCODEC_HOST_DEV inline mat<2, 2, U> inverse(mat<2, 2, T> A) {
