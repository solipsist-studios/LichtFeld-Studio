...
namespace httplib {
    ...
    public:
        ...
        Result(std::unique_ptr<Response>&& res, Error err, Headers&& request_headers,
            ...
            : res_(std::move(res)), err_(err), request_headers_(std::move(request_headers)), ssl_error_(ssl_error), ssl_openssl_error_(ssl_openssl_error) {}
#endif
        // Response
...
#define USE_IF2IP
    ...
    namespace detail {
        ...
        inline SocketStream::SocketStream(
            ...
            time_t max_timeout_msec,
            std::chrono::time_point<std::chrono::steady_clock> start_time)
            : sock_(sock), read_timeout_sec_(read_timeout_sec), read_timeout_usec_(read_timeout_usec), write_timeout_sec_(write_timeout_sec), write_timeout_usec_(write_timeout_usec), max_timeout_msec_(max_timeout_msec), start_time_(start_time), read_buff_(read_buff_size_, 0) {}
    ...
    inline ClientImpl::ClientImpl(const std::string& host, int port,
                                  const std::string& client_cert_path,
                                  const std::string& client_key_path)
        : host_(detail::escape_abstract_namespace_unix_domain(host)), port_(port), host_and_port_(detail::make_host_and_port_string(host_, port, ClientImpl::is_ssl())), client_cert_path_(client_cert_path), client_key_path_(client_key_path) {}
 ...
 */
    ...
    namespace detail {
        ...
        inline SSLSocketStream::SSLSocketStream(
            ...
            time_t max_timeout_msec,
            std::chrono::time_point<std::chrono::steady_clock> start_time)
            : sock_(sock), ssl_(ssl), read_timeout_sec_(read_timeout_sec), read_timeout_usec_(read_timeout_usec), write_timeout_sec_(write_timeout_sec), write_timeout_usec_(write_timeout_usec), max_timeout_msec_(max_timeout_msec), start_time_(start_time) {
    ...
    namespace stream {
        ...
        public:
            ...
            Result(Result&& other) noexcept
                : handle_(std::move(other.handle_)), buffer_(std::move(other.buffer_)), current_size_(other.current_size_), chunk_size_(other.chunk_size_), finished_(other.finished_) {
